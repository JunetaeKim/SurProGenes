...
if __name__ == "__main__":
    ...
    for j in range(1, 6):
        ...
        CosModel,_ = SetModel(NormDismInd, TrIndEmbeddMask, IndN, FeatN, ReferencePatIDLong, ReferencePatIDShort)
        Adam = tf.keras.optimizers.Adam( learning_rate=0.00025)
        CosModel.compile(loss='mse', optimizer=Adam, metrics={"OutVal":'mse' })

        MSavePoint = ModelCheckpoint(filepath='./ModelResults/M02_RCFR_NoGROM_S'+str(j)+  '_Epo'+'{epoch:02d}'+".hdf5", save_weights_only=True, monitor='loss', mode='min',  save_best_only=False)
