...
def SetModel (ExogDTL, ExogPRM, IndN, FeatN, LOCPEM_lr, LOCPEM_hr):
    ...
    GLDXY = tf.matmul(GLD, GLD, transpose_b=True)
    GLD2 = tf.reduce_sum(GLD**2, axis=-1)
    GLDDist = tf.maximum(GLD2[:,None] + GLD2[None] -2*GLDXY, K.epsilon()) 
    GLD_eud = tf.math.sqrt(GLDDist)
    GED = FeatEmbeddAssoTheta/np.pi
    RL_gene = tf.reduce_mean( tf.maximum((GED - GLD_eud)**2 - 0.025, K.epsilon()) ) 

    ## Constrastive loss COS
    FLossBtwCL = tf.matmul(GCMNorm, GCMNorm, transpose_b=True)
    ...
    return RunModel, [InpInd, InpFeat, PEM, PEM_lr, GEM, PCM, GCM, ICosCLSim, FCosCLSim]
